...
void distance()
{
	...
	for (int j2 = 1; j2 < n + 1; j2 = j2 + 1) {
		...
		//if two rooms are adjacent then their distance is one
		distance[i1][j2][0] = 1;
	    else
		distance[i1][j2][0] = 5000;
	    //initially we consider if two rooms are not adjacent then their distance is infinite
	}
    }
 
 
    //array path[][][] gives the immediate predecessor room from room i to room j on the shortest path
    for (int i1 = 1; i1 < n + 1; i1 = i1 + 1) {
	for (int j2 = 1; j2 < n + 1; j2 = j2 + 1) {
    ...
    for (int i1 = 1; i1 < n + 1; i1 = i1 + 1) {	//distance between roomi to roomi is zero
	distance[i1][i1][0] = 0;
    }
