...
void adjacencypairs()
{
	...
	for (int k9 = 0; k9 < no_Groups; k9 = k9 + 1) {	//this consider each group one by one
	    ...
	    //these variables will be used in calculation for width and height of rectangular frame
	    t5 = 0; t1 = 0; d = 0; d1 = 0;
 
	    k3 = Grnumber[k9];	//gives group number
	    no_rects = (v[k3 + 1] - v[k3]);	//number of rooms in corresponding group
 
           //this part calculates width and height of frame for central group and then
		...
		if (0 == spiral[0] || 1 == spiral[0] || 4 == spiral[0] || 5 == spiral[0]) {
		    ...
		    adjacencycal1();	//calculates width and height of rectangular frame for group 1
		    if (k8 == 0)
		...
		if (2 == spiral[0] || 3 == spiral[0] || 6 == spiral[0] || 7 == spiral[0]) {
            ...
            //this part calculates width and height of frame for other groups and then
		...
		if (k3 == Grnumber[j1 + 1]) {	//considering 2nd group
			...
			adjacencycal1();	//calculates width and height of rectangular frame for group two
		...
		if (k3 == Grnumber[1] && (3 == spiral[0] || 6 == spiral[0])) {
            ...
            //this part obtains the members of group 1 which can be adjacent to members of group 3 and their widths
	    if (k8 == 1) {
		if (k3 == Grnumber[0]) {	//considering group1
		    if (0 == spiral[0]) {	//when 1st spiral is used
			adjacencyLength(2, 2);
			//gives members of group1 which can be adjacent to members of group3 and their widths
		    }
		    if (1 == spiral[0]) {
			adjacencyLength(4, 2);
		    }
			...
			adjacencyLength(3, 1);;
                ...
                //their widths and then adjacent members of these two groups
		if (k3 == Grnumber[2] && (0 == spiral[0] || 5 == spiral[0])) {
			...
			adjacencyLength(4, 3);
			//gives which member of group3 can be adjacent to members of group1 and their widths
			findingadjacencies(3, 1);
		...
		if (k3 == Grnumber[3] && (3 == spiral[0] || 6 == spiral[0])) {
            ...
            //this part obtains the members of group 1 which can be adjacent to members of group 5 and their widths
	    if (k8 == 3) {
		if (k3 == Grnumber[0]) {	//considering group1
		    if (0 == spiral[0]) {	//when spiral 1 is used for group1
			adjacencyLength(4, 1);
			//gives members of group1 which can be adjacent to members of group5 and their width
		    }
		    if (1 == spiral[0]) {
			adjacencyLength(2, 1);
		    }
			...
			adjacencyLength(1, 2);
                ...
                //and their widths and then adjacent members of these two groups
		if (k3 == Grnumber[4] && (0 == spiral[0] || 5 == spiral[0])) {
			...
			adjacencyLength(2, 4);
			//gives members of group5 which can be adjacent to members of group1 and their width
...
//this function calculates width and height of the rectangular frame when spiral 1 or 2 or 5 or 6 is used for any group 
void adjacencycal1()
{
 
    H_frame[t1] = height[members_grp[k3][0]];
    //it saves height of first room as H_frame[0]
    t1++;
 
    if (length[members_grp[k3][1]] > length[members_grp[k3][0]]) {
	//bigger width among first and second room is saved as L_frame[0]
	...
	if (j % 4 == 1) {	//its gives height of rectangular frame when any group has 4*n+3 spaces where n starts from zero
	    ...
	    //it sum's the height of last two rooms (or sum's height of last room and sum of heights calculated in case j%4==3)
	    t1++;
 
	    if (height[members_grp[k3][i]] > H_frame[t1 - 1]) {
		//if height of this room > sum of heights calculated above
		H_frame[t1 - 1] = height[members_grp[k3][i]];
		//it save height of this room as sum of heights calculated above
	    }
	}
 
 
	if (j % 4 == 2) {	//its gives width of rectangular frame when any group has 4*n+4 spaces
 
	    L_frame[t5] = L_frame[t5 - 1] + length[members_grp[k3][i - 1]];
	    //here it sum's the width of last two rooms 
	    t5++;
 
	    if (length[members_grp[k3][i]] > L_frame[t5 - 1]) {
		//if width of this room > sum of widths calculated above       
		L_frame[t5 - 1] = length[members_grp[k3][i]];
		//it save width of this room as sum of widths calculated above 
	    }
	}
	...
	if (j % 4 == 3) {	//its gives height of rectangular frame when any group has 4*n+5 spaces
	    ...
	    //here it sum's the height of last rooms and sum of heights calculated in 1st case when j%4==1
	    t1++;
	...
	if (j % 4 == 0) {	//its gives width of rectangular frame when any group has 4*n+6 spaces
 
	    L_frame[t5] = L_frame[t5 - 1] + length[members_grp[k3][i - 1]];
	    //here it sum's width of last room and sum of widths calculated in case j%4==2
	    t5++;
...
//this function calculates width and height of the rectangular frame when spiral 3 or 4 or 7 or 8 is used for any group
void adjacencycal2()
{
 
    L_frame[t5] = length[members_grp[k3][0]];	//it saves height of first room as H_frame[0]
    t5++;
	...
	if (j % 4 == 1) {	//its gives width of rectangular frame when any group has 4*n+3 spaces
	...
	if (j % 4 == 3) {	//its gives width of rectangular frame when any group has 4*n+5 spaces
...
void adjacency1()
{
    if (no_rects == 2) {	//if number of rooms is two then both are adjacent to each other
    ...
    if (no_rects == 3) {	//if number of rooms  is 3 then first one is adjacent to 2nd and 3rd room
    ...
    if (no_rects > 3) {
	//if number of rooms is more than 3 then first room is adjacent to 2nd,3rd,4th and 5th room
	...
	//it starts from 2nd room and each ith room is adjacent to its next room
    ...
    for (int j5 = 1; j5 < no_rects - 1; j5 = j5 + 1) {
	//it starts from 2nd room and each ith room is adjacent to its i+3th room
	if ((j5 + 3) < no_rects) {
	    adjacentpair1[nbrofpairs] = members_grp1[k3][j5];
	...
	//it starts from 2nd room and each ith room is adjacent to i+4th room
	if ((j5 + 4) < no_rects) {
	    adjacentpair1[nbrofpairs] = members_grp1[k3][j5];
...
int e1 = 0, e2 = 0, e3 = 0;			
int e4 = 0; //gives number of rooms of group 1
//these variables are members of group2 or group3 or group4 or group5 which can be adjacent to members of group1
int f1 = 0, f2 = 0, f3 = 0;			
int f4 = 0; //gives number of rooms of group2 or group3 or group4 or group5
//these variables gives width or height of members of group1 which can be adjacent to members of group2 or group3 or group4 or group5
float g1 = 0, g2 = 0, g3 = 0;	
//these variables gives width or height of members of group2 or group3 or group4 or group5 which can be adjacent to members of group1		
float h1 = 0, h2 = 0, h3 = 0;			
 
//In these function given below, instead of height or width of rooms sometimes we are considering values of k1[] or k6[] calculated above
//which is height or width or that room including inner extra space
...
void adjacencyLength(int p3, int p4)
{
	...
	int r1;
	float s1;
	r1 = members_grp1[k3][no_rects - 1];	//this is room number
	s1 = length[members_grp[k3][no_rects - 1]]; //width of this room
	if (p4 == 1 || p4 == 2) {
	    e1 = r1;
	    g1 = s1;
	}
