...
{ 
    ...
    changingroom();	        //change location of a space from one group to other
    arrangingsizes();		//arrange members in each group according to their sizes in ascending order
    plusshape();		//gives plus shape floor plan
    adjacencypairs();		//obtains final adjacency pairs
    adjacencymatrix();		//obtains adjacency matrix, degree of all rooms
    eigenvalue();		//gives eigen values and characteristic function of adjacency matrix
    graph();		        //draws graph
    distance();			//gives distance and shortest path between all rooms
    cutvertex();		//gives cut vertex and pair cut vertex of graph
    eccentricity();		//gives eccentricites of all rooms, radius, diameter and center
    MOI();		        //gives 1st order moment and moment of inertia of all rooms
   ...
   //prints following things on the screen
    PFont myFont = createFont("Times", 24);
    textFont(myFont);
    text("Plus shape floor plan ", 180, 30);
    text("Graph ", 800, 25);
    PFont myFont1 = createFont("Times", 20);
    textFont(myFont1);
    text("Total Area = " + nf(totalarea, 0, 2), 10, 590);
    text("Area of C & T = " + nf((totalarea - area_rects), 0, 2), 10, 620);
    text("Max(MOI) = " + nf(MOI1[1], 0, 2), 10, 650);
    text("Min(MOI) = " + nf(MOI1[n], 0, 2), 10, 680);
